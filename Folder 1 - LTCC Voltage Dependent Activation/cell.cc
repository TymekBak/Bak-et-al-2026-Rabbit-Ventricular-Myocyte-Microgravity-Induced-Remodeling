...
tropi(y[24]), trops(y[25]) {
  ...
  //The default SR calcium concentrations are set to zero.
  cnsr = 0;// NSR load
  cjsr = 0;// average JSR load
  ...
  //The Euler Method step size is reduced.
  dt = 0.01;
  ...
  //The potassium concentrations are set to near-zero values.
  ki = 0.000000001;// mM  internal K
  ko = 0.000000001;//mM    external K
...
double CCell::pacex(double st) {
  ...
  jd = (cs - ci) / taud;//diffusion from submembrane to myoplasm
  // Troponin kinetics
  const double xkon = 0.0327;
  const double xkoff = 0.0196;
  const double btrop = 70.0;
  xbi = xkon*ci*(btrop - tropi) - xkoff*tropi;
  xbs = xkon*cs*(btrop - trops) - xkoff*trops;
  ...
  jca = gca*po*rxa;// Ca current in micro M/ms

  double dcs = comp_inst_buffer(cs)*(50.0*(xir - jd - jca + jnaca) - xbs);
  dci = comp_inst_buffer(ci)*(jd - jup + jleak - xbi);
  dcnsr = -xir + jup - jleak;// SR load dynamics 
  ...
  //The cytoplasmic calcium concentration is fixed at its preset value.
  ci += 0;
  cnsr += dcnsr*dtx;

  //The ryanodine receptor calcium flux is fixed at its preset value.
  xir += 0;
  cjsr += dcjsr*dtx;
  ...
  //The intracellular sodium concentration is fixed at its preset value.
  nai += 0;
  // -------- dV/dt ------------------------------------
...
double CCell::spark_rate() {
  double po = comp_icalpo();
  double rxa = comp_rxa(cs/1000);  
  const double ay = 0.05;
  double sparkV = exp(-ay*(v + 30)) / (1. + exp(-ay*(v + 30)));
  return gRyR*po*fabs(rxa)*sparkV;
}
