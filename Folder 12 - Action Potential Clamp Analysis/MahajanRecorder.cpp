...
//10. Calcium Transient Decay Interval (Peak to 50%)
//11. Calcium Transient Time-to-Peak
...
int main(int argc, char *argv[]) {
    ...
    //Load the input threshold to increase the output resolution near the bifurcation point.
    double Threshold;
    char *ThresholdFile = argv[4];
    FILE *fp3;
    fp3 = fopen( ThresholdFile, "r");
    fscanf(fp3, "%lf", &Threshold);
    fclose(fp3);
    ...
    //Loop Through BCL Values of 400ms to 200ms
    double PCL = 400;

    while(PCL >= 200){
        //Load the AP clamp and prepare it for the simulation.
        int PCL_int = round(PCL);
        string APClampFile = "Baseline AP Correction/" + std::to_string((PCL_int)) + "A.txt";
        const char *APClampFile2 = APClampFile.c_str();
        int PCL_index = round(PCL);
        double APClamp[PCL_index + 1];
        cout << "Voltage Trace: " << endl;
        FILE *fvm;
        fvm = std::fopen(APClampFile2, "r");
        for ( int idy = 0; idy < PCL; idy++ ) {
            fscanf (fvm, "%lf", &APClamp[idy]);
        }
        APClamp[PCL_index] = APClamp[0];
        fclose(fvm);
        ...
        double stim = 0;
        int n = 0;
        int n_total = ceil(1000000/PCL);
        if(abs(Threshold - PCL) < 20){
            n_total = ceil(1000000/PCL);
        }
        ...
        while(t<(PCL*n_total)) {
            //Apply the AP clamp by directly replacing the membrane potential. Advance forwards in time by dt regardless of whether the cell was stimulated.
            VM.pace(stim);
            int APClamp_Index = floor(fmod(t,PCL));
            if(PCL >= 400){
                VM.v = APClamp[APClamp_Index] + (fmod(t,PCL) - floor(fmod(t,PCL)))*(APClamp[APClamp_Index + 1] - APClamp[APClamp_Index]);
            }
            else{
                VM.v = APClamp[APClamp_Index] + (fmod(t,PCL) - floor(fmod(t,PCL)))*(APClamp[APClamp_Index + 1] - APClamp[APClamp_Index]);
            }
            ...
            //Extract the model parameter traces for the last 10000ms with a 1ms sampling rate
