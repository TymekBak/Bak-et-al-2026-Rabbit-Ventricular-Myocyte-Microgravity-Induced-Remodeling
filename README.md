...
•	**CalciumTransientAnalysisFollowUp.py:** This file is used to visually inspect the individuals that failed in the simulation from this folder. Individual 15 is an example of a properly behaving individual.\
...
•	**CalciumTransientAnalysisFollowUp.py:** This file is used to visually inspect the individuals that failed in the simulation from this folder. This script omits the six individuals that were missed due to a typo and a significant part of the individuals which were already analyzed in *Folder 6*.\
...
This folder is needed to follow-up on the results from *Folders 6-7* and correct the alternans thresholds for specific cases. As such, this simulation is needed to finish Figure 11. This part of the project generates a high-resolution bifurcation diagram which is then  examined to find the threshold for alternans initiation. The results are further examined to determine if the individual should be removed from the study or if its alternans thresholds need to be updated. Note that this folder used an older version of the code for computing the calcium transient decay interval (CaTD50). This method breaks down when alternans occur; however, CaTD50 is only used for steady-state verification meaning that these values are still useable on account of their consistent evaluation.\
*The following elements are added to this folder:*\
•	**Bifurcation Diagrams:** This subfolder is used to store the bifurcation diagrams that are generated by *ThresholdSearch.m*.\
•	**TimeCourse:** This subfolder is used to store the action potential and calcium transient traces that are generated by *ThresholdSearch.m*. These traces can be studied manually to determine whether the individual should be kept or not. Criteria for removal include a sudden onset of 2:1 capture and less than 50% recovery in the calcium transient.\
